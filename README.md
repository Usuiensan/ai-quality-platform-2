...
### ステップ 2: 仮想環境の作成とライブラリのインストール（初回のみ）

Pythonがインストールされている環境で、仮想環境を作成して有効化し、必要なライブラリをインストールします。

```powershell
# 1. 仮想環境 (.venv) を作成
python -m venv .venv

# 2. 仮想環境を有効化 (PowerShell の場合)
...
※ **注意**: 仮想環境が有効化されている（プロンプトの先頭に `(.venv)` が表示されている）状態で実行してください。
```powershell
...
プログラムが正しく動いているかを検証する自動テストを実行します。（仮想環境が有効化されている状態で実行してください）
```powershell
