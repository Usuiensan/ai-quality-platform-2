...
def run_autofix(
    ...
    *,
    ...
    approval_callback: Callable[[str, Provider, str], bool] | None = None,
    ...
    for round_no in range(1, max_rounds + 1):
        ...
        round_changed = _apply_fixes(root, fixable, provider, fallback_provider, approval_callback)
        if round_changed == 0:
        ...
        if current_reviews:
            ...
            if next_fixable and approval_callback and round_no < max_rounds:
                # We use the text from the first fixable finding's file as a proxy for the size.
                text_sample = ""
                if next_fixable[0].file:
                    p = root / next_fixable[0].file
                    if p.exists():
                        text_sample = p.read_text(encoding="utf-8")
                
                # Assume we use the main provider for the next round
                active_provider = provider if provider else fallback_provider
                if active_provider:
                    if not approval_callback(f"Autofix Round {round_no + 1}", active_provider, text_sample):
...
def _apply_fixes(root: Path, findings, provider: Provider | None = None, fallback_provider: Provider | None = None, approval_callback: Callable[[str, Provider, str], bool] | None = None) -> int:
    changed_lines = 0
    for finding in findings:
        changed_lines += _apply_single_fix(root, finding, provider, fallback_provider, approval_callback)
...
def _apply_single_fix(root: Path, finding, provider: Provider | None = None, fallback_provider: Provider | None = None, approval_callback: Callable[[str, Provider, str], bool] | None = None) -> int:
    ...
    if fallback_provider is not None:
        if approval_callback:
            if not approval_callback("Fallback Model Retry", fallback_provider, text):
                return 0
