...
def review_requirements(diff_text: str, issue_text: str = "", pr_text: str = "") -> ReviewResult:
    result = _base_result("requirements")
    combined = f"{diff_text}\n{issue_text}\n{pr_text}".lower()
    findings: list[Finding] = []
    if "breaking change" in combined or "破壊" in combined:
        findings.append(
            Finding(
                id="REQ-001",
                severity="high",
...
def final_audit(reviews: list[ReviewResult], diff_text: str, ci_text: str = "") -> ReviewResult:
